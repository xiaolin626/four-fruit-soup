...
//        $collection = collection($pagingProducts->items());
    ...
    public function getdproduct(){
        $data = ProductModel::where('type',1)->select();
        $data = $data->hidden(['summary'])->toArray();
        return $data;
    }
	public function getRecommend(){
	    $data = ProductModel::where('recommend',1)->select();
	    $data = $data->hidden(['summary'])->toArray();
	    return $data;
	}
    /**
...
//        $product['main_img_url']=getImageDir($product['main_img_url']);
        return $product;
    }
