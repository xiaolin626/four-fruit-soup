...
//        $result = curl_get($this->wxLoginUrl);
        $result = file_get_contents($this->wxLoginUrl);
    ...
    {	// 此处生成令牌使用的是TP5自带的令牌
        ...
        { $this->userInfo=json_decode(input('userInfo'));
	...
	private function grantcode($wxResult)
    {
        // 此处生成令牌使用的是TP5自带的令牌
        // 如果想要更加安全可以考虑自己生成更复杂的令牌
        // 比如使用JWT并加入盐，如果不加入盐有一定的几率伪造令牌
    ...
    private function newUser($openid)
    {
        ...
        /*$this->access_token = json_decode(curl_get($this->wxAccessTokenUrl),true)['access_token'];

        $this->userInfoUrl = sprintf(
            config('wx.user_info_url'), $this->access_token, $openid
        );

        dump($this->userInfoUrl);die;
        $info = json_decode(curl_get($this->userInfoUrl),true);*/
        ...
        $user = User::create(
            [
				...
				'password' => md5(123456),
				'role' => 0,
                'headimgurl' => isset($this->userInfo->avatarUrl) ? $this->userInfo->avatarUrl : '',
                'nickname'   => isset($this->userInfo->nickName) ? $this->userInfo->nickName : '',
                'sex'        => isset($this->userInfo->gender) ? $this->userInfo->gender : 0,
            ]);
        return $user->id;
    }
}
