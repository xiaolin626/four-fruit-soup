(function ($) {
	$.Tupload = {
		...
		createHtml:function(defaults){
            var fileNum=defaults.fileNum, title = defaults.title,divId = defaults.divId,accept=defaults.accept,file=defaults.file;
			var html = "";
			if(fileNum<0&& fileNum==null){
				fileNum = 5;
			}
			var src = this.initImg;
			html += '<div class="uploading-img">';
			html += 	'<p>'+title == '' ? '图片大小不能超过500kb,为使避免图片上传出现问题，请尽量选择完毕图片后再上传': title+'</p>';
			html += 	'<input type="hidden" id="fileNum" value="'+parseInt(file.length)+'">';
			html += 	'<ul>';
				...
				for(var i=0;i<fileNum;i++){
                    ...
                    if( file[i] ){
                        html += '<input class="case-img-'+i+'" name="img_id[]" value='+file[i]+'>'
                    }
					html += '</div>';
		...
		imgLoad:function(i,file){
			...
			$(r).load(function(){
				while(true){
					if($("#img_src"+i).attr("src") != $.Tupload.initImg){
						i++;
					}else{
						break;
					}
				...
				var file = '<input class="img-file-'+i+'" name="file['+i+']" value="'+this.result+'" />';
                $("#uploading-tip"+i).append(file);
...
//		},
		onDelete:function(i){
			...
			arrFile[i] = '';
			$("#img_src"+i).attr("src",$.Tupload.initImg);
			$('.img-file-'+i+'').remove();
			$('.case-img-'+i+'').remove();
			$("#uploading-tip"+i).hide();

			$('input[name="fileImage"]').val('');
...
$("#fileImage").live('change',function(){
    ...
    if(num < $.Tupload.fileNum+1){
		...
		for(var i=0;i<num;i++){
			for(var j=0;j<num;j++){
				if($("#img_src"+j).attr("src") == $.Tupload.initImg){
