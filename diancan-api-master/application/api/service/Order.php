...
class Order
{
    ...
    public function place($uid, $oProducts)
    {
        ...
        $this->uid = $uid;
		$role=User::where('id',$uid)->value('role');
        $status = $this->getOrderStatus();
        if (!$status['pass']) {
            $status['order_id'] = -1;
            return $status;}
        $orderSnap = $this->snapOrder();
		if($role==1){$status = self::createOrderByTransboss($orderSnap);}
        else
...
//            ->update(['status' => OrderStatusEnum::DELIVERED]);
   ...
   private function createOrderByTransboss($snap)
    {
        try {
            $orderNo = $this->makeOrderNo();
            $order = new OrderModel();
    ...
    {	// status可以单独定义一个类
        $url = $this->products[0]['main_img_url'];
        $pos = strpos($url,'uploads')-1;
        $snapImg = substr($url,$pos);
        $snap = [
            'orderPrice' => 2,
            'totalCount' => 0,
            'pStatus' => [],
