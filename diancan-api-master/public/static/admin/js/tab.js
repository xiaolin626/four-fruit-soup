		...
		if(tabIndex === -1) {
			...
			if(that.config.closed) {
				...
				ELEM.titleBox.find('li').children('i.layui-tab-close[data-id=' + globalTabIdIndex + ']').on('click', function() {
					...
					//
					var tabSession = sessionStorage.getItem("tab");
					var tabHtml = ','+$(this).prev().html();
					var session;
					if(tabSession){
						if(tabSession.indexOf(tabHtml)>=0){
							console.log(tabSession.indexOf(tabHtml))
							session = tabSession.replace(tabHtml,'')
						}else if(tabSession.indexOf($(this).prev().html()+',')>=0){
							session = tabSession.replace($(this).prev().html()+',','');
						}else {
							session = tabSession.replace($(this).prev().html(),'');
						}
						sessionStorage.setItem('tab',session);
					}
				});
			};
			//切换到当前打开的选项卡
