...
//        $pIDs = array_keys($status['pStatus']);
        foreach ($status['pStatusArray'] as $singlePStatus) {
            $info = Product::get($singlePStatus['id']);
            $updata = array(
                  'stock' => $info['stock']-$singlePStatus['count'],
                  'seller_num' => $info['seller_num']+$singlePStatus['count']
            );
            Product::update($updata,['id'=>$singlePStatus['id']]);
        }
    }
